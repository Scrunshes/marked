...
function mediaParser(provider, url) {
  if (provider === 'youtube') {
    ...
    if (youtubeMatch && youtubeMatch[7].length == 11) {
      return '<div class="video-container"><iframe width="1280" height="720" src="https://www.youtube-nocookie.com/embed/'+ youtubeMatch[7] +'" frameborder="0" allowfullscreen></iframe></div>';
    } else {
      return undefined;
    }
  }
    ...
    if (vimeoMatch && vimeoMatch[1].length == 9) {
      return '<div class="video-container"><iframe src="//player.vimeo.com/video/' + vimeoMatch[1] + '" width="1280" height="720" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe></div>'
    } else {
      return undefined;
    }
  }
...
InlineLexer.prototype.outputLink = function(cap, link) {
  var href = escape(link.href)
    ...
    , videoDomains = ['youtube', 'vimeo', 'dailymotion']
    , domain = ''
    , domainMatching = '';

    if (cap[0].charAt(0) == '!') {
      var url = escape(cap[1]);
      domainMatching = url.match(/((https|http):\/\/)[www.]*[\w]+.(?=\.[com|be|fr|co|gov|net|org|co.uk|])+/);
      if (domainMatching != null) {
        domain = domainMatching[0].replace(/(http|https):\/\/(www.)?/, '');
      }
      if (_.contains(videoDomains, domain)) {
        return this.renderer.video(domain, href);
      }
      else {
...
Renderer.prototype.video = function(domain, href) {

  return mediaParser(domain, href);
};
